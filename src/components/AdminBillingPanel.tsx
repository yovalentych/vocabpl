...
import { CurrencyDollar, FloppyDisk } from "@phosphor-icons/react/dist/ssr";
    ...
    <div className="rounded-3xl border border-ink/10 bg-paper/80 p-6 shadow-soft">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <CurrencyDollar size={24} weight="bold" className="text-terracotta" />
          <div>
            <h2 className="text-2xl font-semibold">{t.admin.billingTitle}</h2>
            <p className="mt-1 text-sm text-ink/60">{t.admin.billingSubtitle}</p>
          </div>
        </div>
        <label className="flex items-center gap-2 rounded-full border border-ink/10 bg-paper/70 px-4 py-2 text-xs font-semibold text-ink/70 transition hover:bg-paper cursor-pointer">
          <input
            type="checkbox"
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        {saved === "done" && <span className="flex items-center gap-1 text-xs font-semibold text-moss">✓ {t.admin.billingSaved}</span>}
        {saved === "error" && <span className="flex items-center gap-1 text-xs font-semibold text-terracotta">✗ {t.admin.billingError}</span>}
      </div>
    </div>
  );
}
