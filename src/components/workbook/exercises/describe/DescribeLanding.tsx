...
import { Image, Sparkle, BookOpen, TrendUp } from "@phosphor-icons/react";
import { useState, useEffect } from "react";

interface DescribeLandingProps {
  onSelectMode: (mode: "classic" | "ai") => void;
}

export default function DescribeLanding({ onSelectMode }: DescribeLandingProps) {
  const { t } = useLocale();
  ...
  useEffect(() => {
    async function loadStats() {
      try {
        const res = await fetch("/api/exercises?exercise=describe");
        if (res.ok) {
          const data = await res.json();
          setStats({
            totalAttempts: data.attempts || 0,
            averageScore: data.avgScore || 0,
            totalDescriptions: data.totalDescriptions || 0,
      ...
      <div className="rounded-3xl border border-gold/20 bg-gold/5 p-8 shadow-soft">
        <div className="flex items-start gap-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gold/20">
            ...
            <Image size={32} weight="fill" className="text-gold" />
          </div>
