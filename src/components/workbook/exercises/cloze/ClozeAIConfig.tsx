  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-moss/20 bg-moss/5 p-6 shadow-soft">
        ...
        <p className="mt-2 text-sm text-ink/60">{t.workbook.clozeAIDesc}</p>
      </div>
      ...
      <div className="rounded-3xl border border-ink/10 bg-paper/80 p-6 shadow-soft">
        ...
        <div className="space-y-3">
          ...
          <input type="range" min={5} max={12} value={sentenceCount} onChange={(e) => setSentenceCount(Number(e.target.value))} className="w-full accent-moss" />
          <div className="flex justify-between text-xs text-ink/40"><span>5</span><span>12</span></div>
        </div>
        <p className="mt-3 text-xs text-ink/50">
          {t.workbook.aiConfigCreditLimit.replace("{max}", "12").replace("{unit}", t.workbook.aiConfigSentencesUnit)}
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-moss/20 bg-moss/5 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-ink/70">{t.workbook.clozeAIInfo}</p>
