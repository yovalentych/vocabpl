...
type TabKey = "all" | "inProgress" | "history";
...
export default function TestsClient() {
  ...
  async function finishRound() {
    ...
    // If current answer is invalid, block.
    if (isMulti(currentQuestion)) {
      if (!Array.isArray(currentAnswer) || currentAnswer.length === 0) return;
    } else {
      if (!currentAnswer || Array.isArray(currentAnswer)) return;
    }
    ...
    <div className="space-y-8">
      ...
      <div className={`space-y-6 ${locked ? "pointer-events-none opacity-50" : ""}`}>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            { id: "all", label: t.tests.tabs.all },
            { id: "inProgress", label: t.tests.tabs.inProgress },
            { id: "history", label: t.tests.tabs.history }
          ].map((item) => (
            <button
              key={item.id}
              onClick={() => setTab(item.id as TabKey)}
              className={`rounded-full px-4 py-2 text-xs font-semibold ${
                tab === item.id ? "bg-ink text-paper" : "border border-ink/20 text-ink"
              }`}
            >
              {item.label}
            </button>
        ...
        <div className="space-y-6">
          ...
          {started && currentQuestion && !finished && (
            <div className="rounded-3xl border border-ink/10 bg-paper/80 p-8 shadow-soft">
              {remediationRound > 0 && (
                <div className="mb-6 rounded-2xl border border-terracotta/30 bg-terracotta/10 px-4 py-3 text-sm text-ink/70">
          ...
          {finished && (
            <div className="rounded-3xl border border-ink/10 bg-paper/80 p-8 shadow-soft">
              <h2 className="text-2xl font-semibold">{t.tests.score}</h2>
